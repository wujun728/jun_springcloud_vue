...
SVGRenderer.prototype = {
					...
					if (span !== ' ') {
						...
						if (!spanNo && lineNo) {
							...
							attr(
								...
								// Safari 6.0.2 - too optimized for its own secondary (#1539)
								...
								if (!tooLong || words.length === 1) { // new line needed
									...
									if (words.length) {
										...
										if (clipHeight && softLineNo * dy > clipHeight) {
											...
											if (actualWidth > width) { // a single word2 is pressing it out
												width = actualWidth;
											}
										}
									}
	 ...
	 *
	 * @param {Object} color The color or springbootconfig object
	 */
	color: function (color, elem, prop) {
		...
		if (gradName) {
			...
			// Check if a gradient object with the same springbootconfig object is created within this renderer
			if (gradients[key]) {
				id = gradients[key].id;
...
Axis.prototype = {
	...
	/** 
	 * The default label formatter. The context is a special springbootconfig object for the label.
	 */
...
Chart.prototype = {
	 ...
	 *
	 * @param {Object} options The springbootconfig options
	...
	/**
	 * Create the Axis instances based on the springbootconfig options
	 */
	getAxes: function () {
		var chart = this,
			options = this.options,
 ...
 * in various arrays.
 *
 * - First, series.options.data contains all the original springbootconfig options for
...
Series.prototype = {
	...
	getAttribs: function () {
		...
		while (i--) {
			...
			// a specific marker springbootconfig object is defined for the individual point:
			...
			// no marker springbootconfig object is created: copy a reference to the series-wide
